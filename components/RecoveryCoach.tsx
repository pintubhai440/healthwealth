...
export const RecoveryCoach: React.FC = () => {
  ...
  const handleVideoSearch = async () => {
      ...
      try {
          ...
          else alert("No specific videos found. Try a different keyword.");
      } catch(e) {
          console.error(e);
      } finally {
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'VIDEO' && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 animate-in fade-in">
            ...
            <p className="text-slate-500 text-sm mb-4">Find 3 best video tutorials.</p>
                        ...
                        <div key={idx} className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between hover:bg-slate-100 transition-colors animate-in slide-in-from-bottom-2">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">Tap to watch on YouTube</p>
                                </div>
                            </div>
                            <a 
                                href={`https://www.youtube.com/results?search_query=${encodeURIComponent(video.search_term)}`} 
                                target="_blank" 
                                rel="noreferrer"
                                className="bg-red-600 text-white p-2 rounded-lg hover:bg-red-700 transition-colors flex items-center gap-1 text-xs font-bold px-3"
                            >
                ...
                <div className="space-y-4">
                    ...
                    <div className="relative aspect-video bg-black rounded-xl overflow-hidden border border-slate-700 shadow-2xl">
                        {!coachResult && !isAnalyzing ? (
                             ...
                             // RESULT OVERLAY
                             <div className="absolute inset-0 bg-slate-900/95 p-6 flex flex-col items-center justify-center text-center animate-in zoom-in-95 overflow-y-auto">
